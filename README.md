...
> The current dataset contains **33** tracks/maps with a total size of approximately **190 MB**. The data is stored mainly in `PNG`, `YAML`, `CSV`, and `JSON` formats, making it easy to use with Python, ROS-style map pipelines, Roboracer/f110-like environments, and custom simulators.
...
- **33 tracks**: a diverse set of benchmark-ready tracks, ranging from F1-style circuits to indoor and warehouse-like maps.
...
- Track names are kept as originally stored. Some names are lowercase or abbreviated, such as `berlin`, `f`, `icra`, `mtl`, and `overtake_map`.
